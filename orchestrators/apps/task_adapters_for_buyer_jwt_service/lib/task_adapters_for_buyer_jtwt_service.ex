defmodule TaskAdaptersForBuyerJwtService do
  @moduledoc false
  ...
  def create(email, password, id) do
    case Node.connect(Application.get_env(:task_adapters_for_buyer_jwt_service, :service_jwt)[:remote_node]) do
      :false -> {:error, ServiceUnavailableError.new("JWT")}
      :ignored -> {:error, ServiceUnavailableError.new("JWT")}
      :true ->
        task = Task.Supervisor.async(
          Application.get_env(:task_adapters_for_buyer_jwt_service, :service_jwt)[:remote_supervisor],
          Application.get_env(:task_adapters_for_buyer_jwt_service, :service_jwt)[:remote_module],
          :create,
          [
            email,
            password,
            id,
            Application.get_env(:task_adapters_for_buyer_jwt_service, :service_jwt)[:secret_key],
            Application.get_env(:task_adapters_for_buyer_jwt_service, :service_jwt)[:secret_ex_key]
          ]
        )
