defmodule Core.ApplicationLayer.UpdatingService do
  ...
  @spec update(binary(), BuyerEntity.updating_dto(), GettingPort.t(), UpdatingPort.t()) :: UpdatingUseCase.ok() | UpdatingUseCase.error()
  def update(maybe_id, updating_dto, getting_port, updating_port) do
    with {:ok, value_id} <- Id.from_origin(maybe_id),
         ...
         {:ok, user_entity} <- BuyerEntity.update(user_entity, updating_dto),
